...
const Home = () => {
  ...
  const uploadWatermarkedFilesToS3 = async (files: File[], retryCount = 3) => {
    try {
      const response = await axios.get(`https://oyster-app-b3323.ondigitalocean.app/presigned_urls_for_watermarked?num_urls=${files.length}`);
  ...
  const uploadOriginalFilesToS3 = async (files: File[], retryCount = 3) => {
    try {
      const response = await axios.get(`https://oyster-app-b3323.ondigitalocean.app/presigned_urls_for_originals?num_urls=${files.length}`);
      const presignedUrls = response.data.urls;
  ...
  const createImagesAndWaves = async (originalUrls: string[], watermarkedUrls: string[]) => {
    try {
      const response = await axios.post('https://oyster-app-b3323.ondigitalocean.app/api/create_images_and_waves/', {
