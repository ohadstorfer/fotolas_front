  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i += batchSize) {
        ...
        const originalUploadPromises = batch.map(file => uploadFilesToS3([file], 'originals'));
        ...
        const watermarkedUploadedUrls = originalUploadedUrls.map(url => {
          ...
          return url
            .replace('surfingram', 'surfingram-watermarked')
            .replace(filename, filename + '-tuvsconverted.0000000.jpg');
        });
      ...
      console.log('Calling createImagesAndWaves with valid URLs');
