...
export function becomePhotographer(credentials: { user: number, about: string,  profile_image: string  }) {
  ...
  const MY_SERVER = 'https://oyster-app-b3323.ondigitalocean.app/photographers/';
...
export function updateDefaultAlbumPricesForImages(credentials: { photographer: number, price_1_to_5: number, price_6_to_50: number, price_51_plus: number }) {
  console.log(credentials);
  const MY_SERVER = `https://oyster-app-b3323.ondigitalocean.app/default-album-prices-update/images/photographer/${credentials.photographer}/`;
  console.log(credentials);
...
export function updateDefaultAlbumPricesForVideos(credentials: { photographer: number, price_1_to_3: number, price_4_to_15: number, price_16_plus: number }) {
  const MY_SERVER = `https://oyster-app-b3323.ondigitalocean.app/default-album-prices-update/videos/${credentials.photographer}/`;
  console.log(credentials);
  return axios.put(MY_SERVER, credentials);
}
...
export const fetchDefaultPricesForImages = async (photographerId: number) => {
  const response = await axios.get(`https://oyster-app-b3323.ondigitalocean.app/default-album-prices/images/photographer/${photographerId}/`);
  return response.data;
};

export const fetchDefaultPricesForVideos = async (photographerId: number) => {
  const response = await axios.get(`https://oyster-app-b3323.ondigitalocean.app/default-album-prices/videos/photographer/${photographerId}/`);
  return response.data;
};